...
try{
  ...
  {const {c,p}=await page(1366,768);await go(p);await openGame(p,11,2);const visibleTargets=await waitForTargetDensity(p,18);await snap(p,'06-crowded-coral-chaos',6,{visibleTargets,minVisibleTargets:18});await c.close()}
