...
async function waitForBoss(p){await p.waitForFunction(()=>document.querySelector('#bossText')?.textContent?.trim()!=='INCOMING',null,{timeout:28000})}
...
try{
  ...
  {
    const a=await page(1366,768);await go(a.p);await a.p.evaluate(()=>{Math.random=()=>0});await openGame(a.p,0);await waitForBoss(a.p);await snap(a.p,'04-reef-run-boss-phase',4);await a.c.close();
    const b=await page(1366,768);await go(b.p);await b.p.evaluate(()=>{Math.random=()=>0});await openGame(b.p,1);await waitForBoss(b.p);await snap(b.p,'05-dragon-depths-boss-phase',5);await b.c.close();
  }
