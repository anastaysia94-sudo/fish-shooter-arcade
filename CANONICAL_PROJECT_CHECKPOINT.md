...
As of 2026-09-25, Android v12 repository-side certification is reproducible on `main`: the debug APK builds, installs and launches in an API 35 Pixel 6 emulator; the trusted production page reaches the Android cabinet readiness marker; portrait/landscape captures and rotation-survival evidence are uploaded; and the repaired release workflow passes both Android release contracts, release lint/unit tests, debug APK, release APK and release AAB builds, package/version verification, deterministic staging, SHA-256 evidence and retained artifact upload. PR #64 passed both the release build and emulator certification before merging as `5468b9cc293b8ebaa35d52723ef67bb636e0a32d`. The green PR run used the explicit unsigned-release-candidate path because production signing secrets were not present in that run. `android-fsa/PHYSICAL_DEVICE_CERTIFICATION_V12.md` now defines the real-device evidence gate and `FSA-Android-v12-debug-installable` is the prepared hardware-test artifact. This is still emulator/repository certification, not physical-device or store certification.
