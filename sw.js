...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    const isAdmin=/\/admin(?:\/|$)/.test(url.pathname),fallback=isAdmin?'./admin/index.html':'./index.html';
    event.respondWith(fetch(event.request).then(response=>{if(response&&response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(fallback,copy));}return response;}).catch(()=>caches.match(fallback)));return;
  }
  ...
  if(/\/admin\/(?:app\.js|styles\.css|index\.html)$/.test(url.pathname)){
    event.respondWith(fetch(event.request).then(response=>{if(response&&response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(event.request,copy));}return response;}).catch(()=>caches.match(event.request)));return;
  }
